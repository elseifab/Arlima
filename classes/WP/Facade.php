...
class Arlima_WP_Facade implements Arlima_CMSInterface
{
    ...
    {
        $sql = $this->prepare("SELECT ID, post_type FROM " . $this->getDBPrefix() . "posts WHERE post_name = %s AND post_type = 'page' ",
            $slug);
        ...
        if (is_array($sidebars) && is_array($wp_registered_widgets)) {
            ...
            foreach ($sidebars as $sidebar => $widgets) {
                ...
                foreach ($widgets as $widget_id) {
                    $index++;
                    if (substr($widget_id, 0,
                            $prefix_len) == Arlima_WP_Widget::WIDGET_PREFIX && !empty($wp_registered_widgets[$widget_id])) {
    ...
    function getImageData($img_id)
    {
        ...
        if ($meta) {
            return array(
                $meta['height'],
                $meta['width'],
                Arlima_WP_ImageVersionManager::uploadDirData('basedir') . '/' . $meta['file']
            );
        } else {
            return array(0, 0, '');
        }
    ...
    function preLoadPosts($post_ids)
    {
        ...
        /** @var wpdb $wpdb */
        global $wpdb;
        foreach ($wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'posts WHERE ID in (' . implode(',',
                $post_ids) . ')') as $post_data) {
