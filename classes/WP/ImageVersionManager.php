...
class Arlima_WP_ImageVersionManager
{
    ...
    {
        $this->attach_id = $id;
        $this->img_quality = is_numeric($plugin_or_img_quality) ? $plugin_or_img_quality : $plugin_or_img_quality->getSetting('image_quality',
            100);
    }
        ...
        if ($file) {
            ...
            if (!empty($meta[self::META_KEY_VERSIONS]) && isset($meta[self::META_KEY_VERSIONS][$max_width])) {
                ...
                if (is_wp_error($editor)) {
                    trigger_error('Failed loading WP image editor for attachment "' . $this->attach_id . '" with message: ' . $editor->get_error_message(),
                        E_USER_ERROR);
                    ...
                    if ($editor->resize($max_width, false)) {
                        ...
                        if (($error = $editor->save(self::uploadDirData('basedir') . '/' . $new_version_file_relative_path)) instanceof WP_Error) {
                            trigger_error('Failed saving resized image for attachment "' . $this->attach_id . '" with message: ' . $error->get_error_message(),
                                E_USER_ERROR);
                            $new_version_file = $file;
                        } else {
                            $version_created_date = $this->saveGeneratedVersion($meta, $new_version_file_relative_path,
                                $max_width);
