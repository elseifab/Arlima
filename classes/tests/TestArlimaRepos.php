...
class TestArlimaRepos extends \WP_UnitTestCase
{
    ...
    function testArticleStuff()
    {
        ...
        $this->assertEquals(true,
            $article->getCreationTime() >= ($time - 1) && $article->getCreationTime() <= ($time + 1));

        $list = self::$list_repo->create('A list', 'some-list');
        self::$ver_repo->create($list, array(Arlima_ListVersionRepository::createArticle(array('title' => 'Heloo'))),
            10);
        $list = self::builder()->id($list->getId())->build();
        ...
        self::$ver_repo->update($list, array(
            ...
            Arlima_ListVersionRepository::createArticle(array(
                'post' => 99,
                'children' => array(
        ...
        self::$ver_repo->updateArticle($articles[0]['id'],
            array('title' => 'yes...', 'options' => array('test2' => 'b')));
        $list = self::builder()->id($list->getId())->build();
