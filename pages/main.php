...
<div id="col-container">
                        ...
                        <select>
                            ...
                            if (isset($settings['limit_access_to_lists']) && $settings['limit_access_to_lists'] == true) {
                                ...
                                if (is_array($allowed_lists)) {
                                    $available_lists = array_filter($available_lists,
                                        function ($list) use ($allowed_lists) {
    ...
    <div id="col-right">
        ...
        <div class="col-wrap">
            ...
            <div id="article-form" class="arlima-postbox">
                ...
                <div class="inside" style="display:none;">
                    ...
                    <div class="settings-buttons">
                        ...
                        <div class="streamer-container template-feature" data-feature="streamer">
                            <select style="display: inline" class="streamer-type-select">
                                ...
                                <?php foreach (apply_filters('arlima_streamer_classes',
                                    array()) as $streamer_class => $label): ?>
                            ...
                            <div class="streamer-images fancybox" id="streamer-images" style="display: none">
                                <?php
                                $streamers = apply_filters('arlima_streamer_images',
                                    array(ARLIMA_PLUGIN_URL . '/images/streamer-example.png'));
                                if (!empty($streamers)):
                                    foreach ($streamers as $streamer): ?>
                                        <img src="<?php echo $streamer ?>" alt="<?php echo $streamer; ?>"
                                             title="<?php echo ucfirst(str_replace(array('-', '_'), ' ',
                                                 pathinfo($streamer, PATHINFO_FILENAME))); ?>"/><br/>
                                    <?php endforeach;
                                endif;
                                ?>
                            </div>
                        ...
                        if (!empty($formats)): ?>
                            <select class="data formats" data-prop="options:format"
                                    data-label="<?php _e('Format', 'arlima') ?>">
                                <option value=""><?php _e('Default', 'arlima') ?></option>
                                <?php foreach ($formats as $format): ?>
                                    <option value="<?php echo $format['class'] ?>"
                                            data-template="<?php if (isset($format['templates']) && is_array($format['templates'])) {
                                                echo join(',', $format['templates']);
                                            } ?>">
                        ...
                        ?>
                        <select class="data templates" data-prop="options:template"
                                data-label="<?php _e('Template', 'arlima') ?>">
                            <option value=""><?php _e('Default', 'arlima') ?></option>
                            <?php
                        ...
                        <select class="data scheduled-settings" data-prop="options:scheduled"
                                data-label="<?php _e('Scheduled', 'arlima') ?>">
                        ...
                        <?php // ADMIN LOCK ?>
                        <select class="data admin-lock" data-prop="options:adminLock"
                                data-label="<?php _e('Admin lock', 'arlima') ?>">
                            <option value=""><?php _e('No') ?></option>
                            <option value="1"><?php _e('Yes') ?></option>
                        </select>
                    ...
                    <div class="image-container template-feature" data-feature="image">
                        ...
                        <div class="image tooltip-left" id="arlima-image"
                             title="<?php _e('Drag images to this container', 'arlima') ?>">
                        ...
                        <div id="arlima-article-attachments" class="fancybox attachments-fancybox"
                             style="width:520px"></div>
                    </div>
                    <div id="image-scissors-popup" style="width: 600px; height: 600px; display: none"></div>

                    <div class="title-container">
                        <input class="pre-title data template-feature" data-feature="title" data-prop="options:preTitle"
                               placeholder="<?php _e('Entry word', 'arlima') ?>"/>
                        <input class="title data" data-prop="title" placeholder="<?php _e('Title', 'arlima') ?>"/>
                        <?php if (!empty($settings['newsbill_tag'])) : ?>
                            <input class="newsbill-tag data template-feature" data-feature="newsbill_tag"
                                   data-prop="options:newsbillTag" placeholder="<?php _e('Newsbill tag', 'arlima') ?>"/>
                        <?php else: ?>
                ...
                <div class="container wp-post">
                    <p>
                        <input type="text" class="search"
                               placeholder="<?php _e('Search for title or post ID...', 'arlima') ?>"/>
            ...
            <?php
            $file_includes = apply_filters('arlima_article_includes',
                array(__('Count Down', 'arlima') => dirname(__FILE__) . '/count-down.php'));
            ksort($file_includes);
            $arlima_file_include = new Arlima_FileInclude();
            ?>
            <div id="arlima-article-file-includes" class="arlima-postbox">
                <div class="collapse-toggle"><br/></div>
                <h3><span><?php _e('File includes', 'arlima') ?></span></h3>
                <div class="inside" style="display:none;">
                    <table class="widefat">
                        <thead>
                        <tr>
        ...
        <tr>
            ...
            <td>
                <input id="schedule-time" type="time"
                       value="<?php echo date('H', current_time('timestamp') + 60 * 60) ?>:00">
            </td>
        </tr>
    </table>
